...
class WC_Bookings_REST_Booking_V2_Controller extends WC_Bookings_REST_CRUD_Controller {
	...
	public function prepare_object_for_response( $booking, $request ) {
		...
		$data = array(
			...
			'attendance_status'        => $booking->get_attendance_status( $context ),
		);
	...
	private function validate_update_request( $request, $creating = false ) {
		...
		if ( $creating ) {
			...
			if ( ! is_numeric( $request['start'] ) || $request['start'] < 0 ) {
				return new WP_Error( 'woocommerce_bookings_rest_invalid_booking', __( 'Start date must be a positive number.', 'woocommerce-bookings' ), array( 'status' => 400 ) );
			}
			if ( ! is_numeric( $request['end'] ) || $request['end'] < 0 ) {
				return new WP_Error( 'woocommerce_bookings_rest_invalid_booking', __( 'End date must be a positive number.', 'woocommerce-bookings' ), array( 'status' => 400 ) );
			}

			if ( $request['start'] < time() ) {
	...
	protected function prepare_object_for_database( $request, $creating = false ) {
		...
		if ( isset( $request['attendance_status'] ) ) {
			$booking->set_attendance_status( $request['attendance_status'] );
		}
		...
		if ( $creating || $requires_availability_check ) {
			...
			$min_check_from = strtotime( "+{$product->get_min_date_value()} {$product->get_min_date_unit()}" );
			$max_check_to   = strtotime( "+{$product->get_max_date_value()} {$product->get_max_date_unit()}" );
			// The end date is exclusive, so we need to check if the end date is greater than or equal to the max check to.
			if ( $booking->get_start() < $min_check_from || $booking->get_end() >= $max_check_to ) {
				return new WP_Error( 'woocommerce_bookings_rest_invalid_duration', __( 'Requested time is out of range.', 'woocommerce-bookings' ), array( 'status' => 400 ) );
			}
		...
		$schema           = array(
			...
			'properties' => array(
				...
				'attendance_status' => array(
					'description' => __( 'Attendance status of the booking.', 'woocommerce-bookings' ),
					'type'        => 'string',
					'enum'        => array( 'attended', 'unattended' ),
					'context'     => array( 'view', 'edit' ),
				),
				'date_created'      => array(
	...
	protected function prepare_objects_query( $request ) {
		...
		// Filter bookings by attendance status.
		if ( isset( $request['attendance_status'] ) ) {
	...
	public function get_collection_params() {
		...
		$params['attendance_status'] = array(
			'description'       => __( 'Filter bookings by attendance status.', 'woocommerce-bookings' ),
			'type'              => 'string',
			'enum'              => array( 'attended', 'unattended' ),
			'validate_callback' => 'rest_validate_request_arg',
		);
