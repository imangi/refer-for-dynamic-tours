...
class WC_Booking_Data_Store extends WC_Data_Store_WP {
	...
	private $booking_meta_key_to_props = array(
		...
		'_booking_date_cancelled'         => 'date_cancelled',
		'_booking_note'                   => 'note',
	...
	*/
	...
	public function create( &$booking ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			// Associate booking ID with order item.
			if ( $booking->get_order_item_id() ) {
				wc_update_order_item_meta( $booking->get_order_item_id(), '_booking_id', array( $id ) );
			}
	...
	public function update( &$booking ) {
		...
		if ( array_intersect( array( 'date_created', 'date_modified', 'status', 'order_id', 'customer_id' ), array_keys( $changes ) ) ) {
			$post_data = array(
				'post_date'     => date( 'Y-m-d H:i:s', $booking->get_date_created( 'edit' ) ), // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
				'post_date_gmt' => get_gmt_from_date( date( 'Y-m-d H:i:s', $booking->get_date_created( 'edit' ) ) ), // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
				'post_status'   => $booking->get_status( 'edit' ),
				'post_author'   => $booking->get_customer_id( 'edit' ),
				'post_parent'   => $booking->get_order_id( 'edit' ),
			);
			...
			if ( isset( $changes['date_modified'] ) && $booking->get_date_modified( 'edit' ) ) {
				$post_data['post_modified']     = date( 'Y-m-d H:i:s', $booking->get_date_modified( 'edit' ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
				$post_data['post_modified_gmt'] = get_gmt_from_date( date( 'Y-m-d H:i:s', $booking->get_date_modified( 'edit' ) ) ); // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
			} else {
	...
	public static function get_booking_ids_from_order_id( $order_id ) {
		...
		return wp_parse_id_list( $wpdb->get_col( "SELECT ID FROM {$wpdb->posts} WHERE post_type = 'wc_booking' AND post_parent IN (" . implode( ',', array_map( 'esc_sql', $order_ids ) ) . ');' ) ); // phpcs:ignore WordPress.DB.PreparedSQL
	}
	...
	public static function get_booking_ids_by( $filters = array() ) {
		...
		return array_filter( wp_parse_id_list( $wpdb->get_col( "{$query_select} {$query_where} {$query_order} {$query_limit};" ) ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- too old code, false positive.
	}
		...
		if ( false === $booking_ids ) {
			...
			if ( $product_or_resource_ids ) {
				...
				foreach ( (array)$product_or_resource_ids as $pid ) {
					if ( in_array( get_post_type( $pid ), wc_booking_get_product_resource_post_types(), true ) ) {
