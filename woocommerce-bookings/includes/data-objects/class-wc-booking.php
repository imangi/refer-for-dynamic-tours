...
class WC_Booking extends WC_Bookings_Data {
	...
	protected function status_transitioned_handler( $from, $to ) {
		...
		if ( $order ) {
			...
			$order_note_id = $order->add_order_note( sprintf( __( 'Booking #%1$d status changed from "%2$s" to "%3$s"', 'woocommerce-bookings' ), $this->get_id(), $from, $to ) );
			// Add booking ID as meta data to the order note.
			if ( $order_note_id ) {
				update_comment_meta( $order_note_id, '_wc_bookings_booking_id', $this->get_id() );
			}
		}
	...
	*/
	...
	public function get_date_cancelled( $context = 'view' ) {
		return (int) $this->get_prop( 'date_cancelled', $context );
	}
	 ...
	 * @since 3.0.0
	 *
	 * @param string|int $timestamp Timestamp in date format or Unix timestamp.
	 */
	public function set_date_cancelled( $timestamp ) {
		$this->set_prop( 'date_cancelled', is_numeric( $timestamp ) ? $timestamp : strtotime( $timestamp ) );
	}
	 ...
	 * @return string
	 */
	public function get_attendance_status( $context = 'view' ) {
		return $this->get_prop( 'attendance_status', $context );
	}
	 ...
	 * @since 3.1.0
	 *
	 * @param string $attendance_status Attendance status to set.
	 */
	public function set_attendance_status( $attendance_status ) {
		$this->set_prop( 'attendance_status', $attendance_status );
	}
	...
	public function get_start_date( $date_format = null, $time_format = null, $local = false ) {
		if ( $this->get_start( 'view', $local ) ) {
			...
			if ( is_null( $time_format ) ) {
				$prefix = is_string( $date_format ) && $date_format ? ', ' : '';
				/**
				 * Filter the time format for the booking start date.
				 *
				 * @since 3.3.0
				 *
				 * @param string $time_format The time format.
				 */
				$time_format = apply_filters( 'woocommerce_bookings_time_format', $prefix . wc_bookings_time_format() );
			}
			if ( $this->is_all_day() ) {
	...
	public function get_end_date( $date_format = null, $time_format = null, $local = false ) {
		if ( $this->get_end( 'view', $local ) ) {
			...
			if ( is_null( $time_format ) ) {
				$prefix = is_string( $date_format ) && $date_format ? ', ' : '';
				/**
				 * Filter the time format for the booking end date.
				 *
				 * @since 3.3.0
				 *
				 * @param string $time_format The time format.
