...
class WC_Bookings_Blocks_Controller {
	...
	public function __construct() {
		...
		new WC_Bookings_Blocks_Team_Member_Block_Bindings();
		new WC_Bookings_Blocks_Order_Confirmation();
		new WC_Bookings_Blocks_Cart_Item_Data();

		// Register block variations.
		add_filter( 'get_block_type_variations', array( $this, 'add_post_featured_image_variations' ), 10, 2 );
	}
	...
	public function hook_add_to_cart_with_options_button_to_modal( $block_content, $block, $instance ) {
		...
		while ( $p->next_tag() ) {
			if ( $p->get_tag() === 'BUTTON' && $p->has_class( 'add_to_cart_button' ) ) {
				$p->set_attribute( 'data-wp-on--click', 'woocommerce-bookings/booking-modal::actions.openModal' );
				$p->set_attribute( 'type', 'button' );
			}
			if ( $p->get_tag() === 'DIV' && $p->has_class( 'continue-shopping' ) ) {
				$p->next_tag( 'a' );
				$p->set_attribute( 'data-wp-on--click', 'woocommerce-bookings/booking-form::actions.addToCartAndContinueShopping' );
				$current_url = home_url( add_query_arg( null, null ) );
				$p->set_attribute( 'href', $current_url );
			}
			if ( $p->get_tag() === 'DIV' && $p->has_class( 'complete-booking' ) ) {
				$p->next_tag( 'a' );
				$p->set_attribute( 'data-wp-on--click', 'woocommerce-bookings/booking-form::actions.addToCartAndCompleteBooking' );
	...
	/**
	 * Adds variations for the core/post-featured-image block.
	 *
	 * @param array         $variations Array of registered variations for a block type.
	 * @param WP_Block_Type $block_type The full block type object.
	 * @return array Array of variations with team member image variation added.
	 */
	public function add_post_featured_image_variations( $variations, $block_type ) {
		if ( 'core/post-featured-image' !== $block_type->name ) {
			return $variations;
		}

		$team_member_variation = array(
			'name'        => 'team-member-image',
			'title'       => __( 'Team Member Image', 'woocommerce-bookings' ),
			'description' => __( 'Display a team member\'s featured image.', 'woocommerce-bookings' ),
			'attributes'  => array(
				'sizeSlug' => 'medium',
				'scale'    => 'cover',
				'metadata' => array(
					'wcBookingsVariation' => 'team-member-image',
				),
			),
			'scope'       => array( 'inserter', 'transform' ),
			'icon'        => 'format-image',
			'isActive'    => array( 'metadata.wcBookingsVariation' ),
		);

		$variations[] = $team_member_variation;
