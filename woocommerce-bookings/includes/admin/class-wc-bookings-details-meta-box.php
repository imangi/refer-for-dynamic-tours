...
class WC_Bookings_Details_Meta_Box {
	...
	public function meta_box_inner( $post ) {
		...
		// Check if attendance status meta exists for old bookings.
		$attendance_status_meta_exists = metadata_exists( 'post', $booking->get_id(), '_booking_attendance_status' );

		// Check if booking is cancelled - hide attendance status if cancelled.
		$is_cancelled = 'cancelled' === $booking->get_status();

		// Hidden booking item meta.
		$hidden_booking_itemmeta = array( '_booking_id' );
			...
			<div id="booking_data" class="panel">
				...
				<div class="booking_data_column_container">
					<div class="booking_data_column">
						...
						<?php if ( ! $attendance_status_meta_exists ) : ?>
							<span class="form-field form-field-wide booking-attendance-status-prompt"<?php echo $is_cancelled ? ' style="display: none;"' : ''; ?>>
								<p>
									<strong><?php esc_html_e( 'Attendance Status not configured', 'woocommerce-bookings' ); ?></strong>
								</p>
								<p class="booking-attendance-status-description">
									<?php esc_html_e( 'This booking was created before the attendance status feature was added.', 'woocommerce-bookings' ); ?>
								</p>
								<p>
									<?php
									$tool_url = admin_url( 'admin.php?page=wc-status&tab=tools' );
									printf(
										/* translators: %s: Link to the tool */
										wp_kses_post( __( 'You can use the <a href="%s" target="_blank" rel="noopener noreferrer">WooCommerce > Status > Tools > Add missing attendance status</a> tool to add attendance status to all bookings at once.', 'woocommerce-bookings' ) ),
										esc_url( $tool_url )
									);
									?>
								</p>
								<button type="button" id="configure-attendance-status-btn" class="button button-secondary">
									<?php esc_html_e( 'Configure Attendance Status', 'woocommerce-bookings' ); ?>
								</button>
							</span>
							<p class="form-field form-field-wide booking-attendance-status-field" style="display: none;">
						<?php else : ?>
							<p class="form-field form-field-wide booking-attendance-status-field"<?php echo $is_cancelled ? ' style="display: none;"' : ''; ?>>
						<?php endif; ?>
							<label for="_booking_attendance_status">
								<?php esc_attr_e( 'Attendance status:', 'woocommerce-bookings' ); ?>
							</label>
							<select id="_booking_attendance_status" name="_booking_attendance_status" class="wc-enhanced-select">
							<?php
							echo '<option value="attended" ' . selected( 'attended', $booking->get_attendance_status(), false ) . '>' . esc_html__( 'Attended', 'woocommerce-bookings' ) . '</option>';
							echo '<option value="unattended" ' . selected( 'unattended', $booking->get_attendance_status(), false ) . '>' . esc_html__( 'Unattended', 'woocommerce-bookings' ) . '</option>';
							?>
							</select>
							<input type="hidden" name="post_status" value="<?php echo esc_attr( $booking->get_attendance_status() ); ?>">
						</p>
						...
						<p class="form-field form-field-wide">
							...
							try {
								...
								foreach ( $order_item->get_formatted_meta_data() as $meta ) :
									if ( in_array( $meta->key, $hidden_booking_itemmeta, true ) ) {
										continue;
									}
									?>
									<tr>
		...
		$handle = 'wc-bookings-details-all-day';
		wp_register_script( $handle, '', array( 'jquery', 'jquery-ui-datepicker' ), false, true ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NoExplicitVersion
				...
				$( function() {
					...
					$( '.date-picker-field' ).datepicker({
						...
						beforeShow: function( input, datePicker ) {
							datePicker.dpDiv.addClass( 'wc-bookings-ui-datepicker-widget' );
						},
						onClose: function( dateText, datePicker ) {
							datePicker.dpDiv.removeClass( 'wc-bookings-ui-datepicker-widget' );
						}
					});
				} );
			} )( jQuery );
		...
		// Select2 handling.
		$handle = 'wc-bookings-details-select2';
		wp_register_script( $handle, '', array( 'jquery', 'selectWoo' ), false, true ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.NoExplicitVersion
		wp_enqueue_script( $handle );
		wp_add_inline_script(
			$handle,
			"
	...
	public function meta_box_save( $post_id, $post ) {
		...
		if ( strtotime( $end_date ) < strtotime( $start_date ) ) {
			WC_Admin_Notices::add_custom_notice(
				...
				'<strong>' . esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ) . '</strong> ' . esc_html__( 'Start date cannot be greater than end date.', 'woocommerce-bookings' )
			);
			return $post_id;
		}
		...
		if ( false === $is_valid_start_date || false === $is_valid_end_date ) {
			WC_Admin_Notices::add_custom_notice(
				...
				'<strong>' . esc_html__( 'WooCommerce Bookings:', 'woocommerce-bookings' ) . '</strong> ' . esc_html__( 'Date should be of the format yyyy-mm-dd and cannot be empty.', 'woocommerce-bookings' )
			);
		...
		$booking->set_props(
			array(
				...
				// phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp.Requested,WordPress.Security.ValidatedSanitizedInput.InputNotValidated
				...
				'attendance_status' => isset( $_POST['_booking_attendance_status'] ) ? wc_clean( wp_unslash( $_POST['_booking_attendance_status'] ) ) : '',
			)
		);
		...
		if ( ! empty( $booking_order_id ) ) {
			...
			foreach ( $order_items as $order_item ) {
				...
				if ( ! $order_item_id ) {
					throw new Exception( esc_html__( 'Error: Could not create item', 'woocommerce-bookings' ) );
				}
