...
/**
 * WC_Bookings_Admin class.
 *
 * Handles booking admin functionality.
 */
class WC_Bookings_Admin {
	...
	public function bookings_debug_tools( $tools ) {
		$bookings_tools = array(
			...
			'add_missing_attendance_status'  => array(
				'name'     => __( 'Add missing attendance status', 'woocommerce-bookings' ),
				'button'   => __( 'Schedule task', 'woocommerce-bookings' ),
				'desc'     => __( 'This tool will schedule a background task to fill in the Attendance status field on all existing bookings that don\'t have one. Completed bookings will be marked as Attended, cancelled bookings as Unattended, and all other statuses as Unattended.', 'woocommerce-bookings' ),
				'callback' => array( 'WC_Bookings_Tools', 'schedule_add_missing_attendance_status' ),
			),
		);
	...
	public function redirect_new_add_booking_url() {
		...
		if ( 'post-new.php' == $pagenow && isset( $_GET['post_type'] ) && 'wc_booking' == $_GET['post_type'] ) {
			wp_safe_redirect( admin_url( 'edit.php?post_type=wc_booking&page=create_booking' ), '301' );
		}
	}
	...
	public static function get_booking_resources() {
		...
		foreach ( $ids as $id ) {
			$resources[] = get_wc_product_booking_resource( $id );
		}
		return $resources;
	}
	...
	public function styles_and_scripts() {
		...
		$params = array(
			...
			'i18n_max_persons_warning'      => esc_js( __( 'Max allowed is set to', 'woocommerce-bookings' ) ),
	...
	public function handle_deleted_bookable_product( $post_id ) {
		...
		if ( ! is_a( $product, 'WC_Product' ) || ! is_wc_booking_product( $product ) ) {
			return;
		}
