...
if ( ! class_exists( 'WC_Bookings' ) ) :
	...
	define( 'WC_BOOKINGS_VERSION', '3.4.0' ); // WRCS: DEFINED_VERSION.
	...
	class WC_Bookings {
		...
		public function __construct() {
			...
			add_action( 'plugins_loaded', array( $this, 'woocommerce_bookings_init' ), 10 );
		}
		 ...
		 * @since 1.13.1
		 */
		public function woocommerce_bookings_init() {
		...
		private function init() {
			...
			// Initialize.
			new WC_Bookings_Core_Compatibility();
			...
			WC_Booking_Order_Manager::instance();
			...
			if ( class_exists( '\WooCommerce\PayPalCommerce\PluginModule' ) ) {
				new WC_Bookings_PayPal_Payments();
			}
			...
			WC_Booking_Cart_Manager::get_instance();
			new WC_Booking_Checkout_Manager();
			new WC_Bookings_REST_API();
			new WC_Booking_Store_API();
			new WC_Bookings_Product_Rest_API();
			...
			add_action( 'init', array( $this, 'show_activation_notice' ), 10 );

			if ( function_exists( 'wp_is_block_theme' ) && wp_is_block_theme() ) {
				new WC_Bookings_Blocks_Controller();
			}
		}
		...
		public function show_activation_notice() {
			if ( false !== get_option( WC_BOOKINGS_ACTIVATION_NOTICE_KEY ) ) {
				...
				if ( class_exists( 'WC_Admin_Notes' ) ) {
					...
					$notice_html = '<strong>' . esc_html__( 'WooCommerce Bookings has been activated!', 'woocommerce-bookings' ) . '</strong><br><br>';
