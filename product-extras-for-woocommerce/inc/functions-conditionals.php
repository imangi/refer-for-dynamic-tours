...
function pewc_get_conditional_field_visibility( $id, $item, $items, $product_id, $posted=array(), $variation_id=null, $cart_item_data=array(), $quantity=0, $group_id=false, $group=false ) {
	...
	if( empty( $conditions ) ) {
		...
		// 3.26.5
		$has_repeatable = ! empty( $group['has_repeatable'] ) ? true : false;
		$repeatable_index = ( $has_repeatable && ! empty( $group['clone_count'] ) ) ? $group['clone_count'] - 1 : 0;
		...
		if( $rules['match'] == 'all' ) {
			...
			foreach( $conditions as $condition ) {
				...
				$condition_group_id = isset( $condition_field_id[2] ) ? $condition_field_id[2] : false; // 3.26.5
				...
				if( is_array( $posted_field ) ) {
					...
					// 3.25.4
					if ( ! empty( $posted[$field . '_is_repeatable'] ) ) {
						// if this is a repeatable field, this could be a text, number, or select field, which need a single value for the condition
						if ( isset( $posted_field[$repeatable_index] ) ) {
							// 3.26.5, if this is a cloned field, get the correct value?
							$posted_field = $posted_field[$repeatable_index];
						} else if ( isset( $posted_field[0] ) ) {
							// get the first value
							$posted_field = $posted_field[0];
						} else {
							$posted_field = '';
						}
					}
				...
				// 3.21.7, cost-based conditions don't work if line_total is false, especially when called in pewc_validate_cart_item_data() and pewc_add_cart_item_data()
				// it's false because $cart_item_data and $cart_item['line_total'] are not yet set
				if ( 'cost' === $field && false === $line_total ) {
					$line_total = pewc_get_line_total_for_cost_condition( $posted, $quantity );
				}
				...
				} else if ( 'log-in-status' === $field ) {

					// 3.21.5
					$log_in_status = is_user_logged_in() ? 'true' : 'false';

					if( $rule == 'is' ) {
						$rules_obtain = $log_in_status === $value;
					} else if( $rule == 'is-not' ) {
						$rules_obtain = ! ( $log_in_status === $value );
					}
...
function pewc_is_group_visible( $group_id, $group, $posted ) {
	...
	// 3.22.0
	$ignored_fields = array( 'log-in-status', 'user-role' );

	foreach( $conditions as $condition ) {

		$field_id = $condition['field'];
		if( in_array( $field_id, $ignored_fields ) ) {
			// Ignore condition e.g. log-in status, user role
			continue;
		}
		...
		// 3.25.4, if source field is repeatable, field_value becomes an array. This could affect pewc_is_field_visible if rule is greater than or less than
		if ( is_array( $field_value ) && ! empty( $posted[$field_id . '_is_repeatable'] ) ) {
			// get only the first value?
			if ( isset( $field_value[0] ) ) {
				$field_value = $field_value[0];
			} else {
				$field_value = ''; // blank for now?
			}
		}
...
/**
 * Remove any groups that are excluded for log-in status or user role
 * @since	3.20.0
 * @version	3.22.0
 */
function pewc_hide_groups_by_condition( $product_extra_groups, $product_id ) {
	
	if( is_array( $product_extra_groups ) ) {
		foreach( $product_extra_groups as $group_id=>$group ) {
			// 3.22.0, combine checking of log-in status and user role
			if( ! pewc_is_group_visible_by_condition( $group_id, $group ) ) {
...
add_filter( 'pewc_filter_product_extra_groups', 'pewc_hide_groups_by_condition', 11, 2 );
...
/**
 * Check if a group is visible according to log-in status or user role
 * @since	3.20.0
 * @version	3.22.0
 */
function pewc_is_group_visible_by_condition( $group_id, $group ) {
	...
	// Iterate through each condition
	// We're only looking at the user's log-in status and role, ignoring other conditions
	$fields_to_check = array( 'log-in-status', 'user-role' );
	foreach( $conditions as $condition ) {
		...
		if ( 'log-in-status' === $field_id ) {
			if( $condition['rule'] == 'is' ) {
				$is_group_visible = $log_in_status === $condition['value'];
			} else if( $condition['rule'] == 'is-not' ) {
				$is_group_visible = ! ( $log_in_status === $condition['value'] );
			}
		} else if ( 'user-role' === $field_id ) {
			$has_role = in_array( $condition['value'], (array) $user->roles );
			if( $condition['rule'] == 'is' ) {
				$is_group_visible = $has_role;
			} else if( $condition['rule'] == 'is-not' ) {
				$is_group_visible = ! $has_role;
			}
		}
		...
		// we have several conditions that can trigger a break in the loop
		if ( 'show' === $action ) {
			if ( ! $is_group_visible && 'all' === $match ) {
				break; // if just one condition fails, the whole condition fails
			} else if ( $is_group_visible && 'any' === $match ) {
				break; // if just one condition succeeds, the whole condition succeeds
			}
		} else {
			// 'hide' === $action
			if ( ! $is_group_visible && 'any' === $match ) {
				break; // if just one condition succeeds, the whole condition succeeds
			} else if ( $is_group_visible && 'all' === $match ) {
				break; // if just one condition fails, the whole condition fails
			}
		}
...
/**
 * Gets a possible line total for checking if field is visible based on cost
 * to-do: on the product page the condition is based on #pewc_total_calc_price, which also includes other add-ons, so there might be some discrepancies here?
 * @since 3.21.7
 */
function pewc_get_line_total_for_cost_condition( $posted, $quantity ) {

	$line_total = false;

	if ( isset( $posted['pewc-product-price'] ) ) {
		$product_price = (float) $posted['pewc-product-price'];
		$line_total = (float) $product_price * $quantity;
	}
