		...
		if( ! $has_migrated ) {
			...
			foreach( $form_obj as $item ) {
				...
				if( strpos( $name, '_product_extra_groups' ) !== false ) {
					...
					// 3.22.0, this function saves multiple group IDs, which may have different Repeatable settings
					if ( is_numeric( $group_id ) ) {
						if ( ! isset( $repeatable_groups[$group_id] ) ) {
							$repeatable_groups[$group_id] = false;
						}
					...
					// 3.22.0
					if( strpos( $name, 'repeatable_limit' ) !== false ) {
						$value = (int) $value;
						update_post_meta( $group_id, 'repeatable_limit', sanitize_text_field( $value ) );
					} else if( strpos( $name, 'repeatable_by_quantity' ) !== false ) {
						update_post_meta( $group_id, 'repeatable_by_quantity', sanitize_text_field( $value ) );
						$repeatable_by_quantity[$group_id] = true; // so that we don't delete this postmeta later
					} else if( strpos( $name, 'repeatable' ) !== false ) {
						update_post_meta( $group_id, 'repeatable', sanitize_text_field( $value ) );
						$repeatable_groups[$group_id] = true; // so that we don't delete this postmeta later
					}
					...
					do_action( 'pewc_globals_extra_metadata', $group_id, $name, $value, $item );

				}

			}

			// 3.22.0, delete postmeta for non-repeatable groups
			if ( ! empty( $repeatable_groups ) ) {
				foreach ( $repeatable_groups as $repgroup_id => $is_repeatable ) {
					if ( ! $is_repeatable ) {
						delete_post_meta( $repgroup_id, 'repeatable' );
					}
				}
			}
			if ( ! empty( $repeatable_by_quantity ) ) {
				foreach ( $repeatable_by_quantity as $repgroup_id => $by_quantity ) {
					if ( ! $by_quantity ) {
						delete_post_meta( $repgroup_id, 'repeatable_by_quantity' );
					}
				}
			}
...
function pewc_filter_product_extra_groups( $product_extra_groups, $product_id ) {
	...
	if( $globals ) {
		...
		// 3.26.9, moved here so that if a global group is added from the Edit Product page, it can retain the sort order in the Global Add-Ons or Global Groups page
		$pewc_global_groups_by_product_array = array();
		$product = wc_get_product( $product_id );
		if( is_object( $product ) ) {
			$pewc_global_groups_by_product = $product->get_meta( 'pewc_global_groups_by_product' );
			if( $pewc_global_groups_by_product ) {
				$pewc_global_groups_by_product_array = explode( ',', $pewc_global_groups_by_product );
			}
		}
		...
		foreach( $globals as $global_id=>$global ) {
			...
			// 3.24.2, only show Published status by default
			if( ! in_array( get_post_status( $global_id ), apply_filters( 'pewc_allowed_global_groups_post_status', array( 'publish' ), $product_id ) ) ) {
				continue;
			}

			// 3.26.9, if added on product-level, add immediately
			// 3.26.11, moved here. This allows global groups with language issues to still appear on the product page
			if ( ! empty( $pewc_global_groups_by_product_array ) && in_array( $global_id, $pewc_global_groups_by_product_array ) ) {
		...
		// 3.21.5, moved this outside of the foreach $globals loop
		// 3.26.9, commented out, moved to above before the foreach loop
		//$product = wc_get_product( $product_id );
		//if( is_object( $product ) ) {
		//	$pewc_global_groups_by_product = $product->get_meta( 'pewc_global_groups_by_product' );
		//	if( $pewc_global_groups_by_product ) {
...
function pewc_show_products_rule( $group_key, $group, $rule, $products, $product_titles ) {
	...
	if( $products ) {
		// 3.20.1, searchable products
		$select = '<select multiple="multiple" class="pewc-rule-field wc-product-search" name="_product_extra_groups_' . esc_attr( $group_key ) . '[global_rules][ids][products][]" data-name="_product_extra_groups_GROUP_KEY[global_rules][ids][products][]" id="_product_extra_groups_' . esc_attr( $group_key ) . '[global_rules][ids][products]">';
		...
		foreach ( $set_products as $id ) {
			$title = isset( $product_titles[$id] ) ? $product_titles[$id] : get_the_title( $id );
			$select .= '<option selected="selected" value="'. $id . '">' . $title . '</option>';
		}
		$select .= '</select>';
	}
	return $select;
...
function pewc_show_categories_rule( $group_key, $group, $rule ) {
	...
	if( $categories ) {
		...
		foreach( $categories as $id ) {
			...
			$selected = ( in_array( $id, $set_categories ) || in_array( $term->slug, $set_categories ) ) ? 'selected="selected"' : ''; // 3.21.4, moved here. Also, on 3.21.3 and earlier, global groups as post types save categories as slugs. This has been updated in 3.21.4 to use term_id, but accommodate them here as well
			$term_name = sprintf(
				'%s (ID: %s)',
				$term->name,
				$term->term_id
