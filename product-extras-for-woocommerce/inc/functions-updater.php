...
function pewc_do_license_activation( $license ) {
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$message = 'An error occurred, please try again. Response code: ' . wp_remote_retrieve_response_code( $response );
		}
		...
		if( isset( $license_data->price_id ) ) {
			update_option( 'pewc_price_id', $license_data->price_id );
		}
...
function pewc_get_general_settings_url() {
	$settings_url = get_admin_url( null, 'admin.php' );
	$settings_url = add_query_arg(
		array(
			'page'		=> 'wc-settings',
