...
function pewc_sanitise_radio_field( $item ) {
	if( ! empty( $item['field_options'] ) ) {
		foreach( $item['field_options'] as $index=>$option ) {
			...
			// 3.26.12, new version. This seems to not be in used, but this is used now in admin/functions-custom-panel.php when saving radio fields
			$item['field_options'][$index]['key'] = str_replace( '-', '_', sanitize_title( pewc_keyify_field ( $item['field_options'][$index]['value'] ) ) );
		}
	}
