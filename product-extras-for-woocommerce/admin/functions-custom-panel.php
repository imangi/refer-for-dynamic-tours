	...
	<div id='pewc_options' class='panel woocommerce_options_panel pewc_panel <?php echo esc_attr( $class ); ?>'>
		...
		<div class="options_group">
			<div class="options-group-inner">
				...
				<p class="pewc-import-export-aou-select-all"><input type="checkbox" id="pewc_select_all_group_for_export"> <?php _e( 'Select all groups', 'pewc' ); ?></p>
			...
			foreach( $_POST as $key=>$value ) {
				...
				if( strpos( $key, '_product_extra_groups_' ) !== false ) {
					...
					if( ! in_array( $group_id, $group_order ) ) {
						...
						// 4.0.3, added condition to prevent PHP warning
						if ( $group_id ) {
						...
						if( ! $field_id ) {
							...
							if( $always_include ) {
								...
								delete_post_meta( $group_id, 'always_include' );
							}

							// 3.22.0
							$repeatable = ! empty( $_POST['_product_extra_groups_' . $group_id]['meta']['repeatable'] ) ? $_POST['_product_extra_groups_' . $group_id]['meta']['repeatable'] : false;
							if( $repeatable ) {
								update_post_meta( $group_id, 'repeatable', true );
							} else {
								delete_post_meta( $group_id, 'repeatable' );
							}
							$repeatable_by_quantity = ! empty( $_POST['_product_extra_groups_' . $group_id]['meta']['repeatable_by_quantity'] ) ? $_POST['_product_extra_groups_' . $group_id]['meta']['repeatable_by_quantity'] : false;
							if( $repeatable_by_quantity ) {
								update_post_meta( $group_id, 'repeatable_by_quantity', true );
							} else {
						...
						foreach( $params as $param ) {
							...
							if( isset( $value[$param] ) ) {
								...
								if ( isset( $value['field_type'] ) && 'radio' === $value['field_type'] && 'field_options' === $param && ! empty( $value[$param] ) ) {
									// 3.26.12, sanitise the radio options
									$value = pewc_sanitise_radio_field( $value );
								}
...
function pewc_field_types() {
	$field_types = array(
		...
		'calendar-list'			=> __( 'Calendar List', 'pewc' ),
 ...
 * @since 3.27.3
 */
function pewc_maybe_admin_page_has_pewc() {
	global $pagenow, $post;

	// Check if we are in the admin area
	if ( is_admin() ) {
		// Check if the current page is 'post.php' (for editing existing posts/products)
		// or 'post-new.php' (for creating new posts/products)
		$pewc_post_types = array( 'product', 'pewc_group' );
		if ( in_array( $pagenow, array( 'post.php', 'post-new.php' ) ) ) {
			// If editing an existing product, verify the post type
			if ( 'post.php' === $pagenow && isset( $_GET['post'] ) ) {
				$post_id = absint( $_GET['post'] );
				$post_type = get_post_type( $post_id );
				if ( in_array( $post_type, $pewc_post_types ) ) {
					return true;
				}
			}
			// If creating a new product, verify the post type
			if ( 'post-new.php' === $pagenow && isset( $_GET['post_type'] ) && $_GET['post_type'] === 'product' ) {
				return true;
			}
		}
	}
...
/**
 * Add our own pewc-data-tip. data-tip is removed when the tooltip is generated, so when cloning new-field-item, the tooltip is not regenerated because of the missing data-tip
 * @since 3.27.3
 */
function pewc_filter_help_tip( $html_tip, $sanitized_tip, $tip, $allow_html ) {

	global $pagenow, $post;

	if ( 'pewc' === $allow_html && false !== strpos( $html_tip, ' data-tip="' ) ) {
		// maybe wc_help_tip is used incorrectly in templates/admin/field-item.php, but let's use it for now to add our own data
		// data-tip is removed by jquery-tiptip, so when the new-field-item is cloned, the tip isn't reinitialized because data-tip is already missing
		$pewc_tip = ' pewc-data-tip="' . $sanitized_tip . '" ';
		$html_tip = str_replace( ' data-tip="', $pewc_tip . ' data-tip="', $html_tip );
	}
	return $html_tip;

}
if ( pewc_maybe_admin_page_has_pewc() ) {
	// only add the filter when we're on the admin Edit Product page or Global Group pages?
	add_filter( 'wc_help_tip', 'pewc_filter_help_tip', 10, 4 );
}
