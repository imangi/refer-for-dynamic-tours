...
function pewc_duplicate_group() {
  ...
  if( ! isset( $_POST['old_group_id'] ) ) {
	...
	if( $group_id ) {
		...
		// 3.22.0
		pewc_duplicate_repeatable_postmeta( $old_group_id, $group_id );
		...
		if ( ! empty( $mapped_fields ) ) {
			...
			foreach ( $new_field_ids as $fid ) {
				...
				// 3.25.4
				do_action( 'pewc_duplicate_group_map_field_ids', $fid, $ftype, $mapped_fields );
			}
		}
 ...
 * @return	String		E.g. 259,265,275
 * 
 * Note: added $add_after since 3.22.1
 */
function pewc_add_field_to_group( $field_id, $group_id, $add_after=false ) {
	// field_ids is an array
	$fields = get_post_meta( $group_id, 'field_ids', true );
	if( ! $fields ) {
		$fields = array( $field_id );
	} else if ( false !== $add_after ) {
		// 3.22.1, add the field after a specific field_id
		$new_fields = array();
		if ( 0 == $add_after ) {
			// add the field to the first of the list
			$new_fields[] = $field_id;
		}
		foreach ( $fields as $curr_field_id ) {
			$new_fields[] = $curr_field_id;
			if ( $curr_field_id == $add_after && ! in_array( $field_id, $new_fields ) ) {
				$new_fields[] = $field_id;
			}
		}
		$fields = $new_fields;
 ...
 * Added force_reset argument @since 3.11.5, this is set to true in pewc_save_globals()
 */
function pewc_reset_all_transients( $force_reset = false ) {
...
/**
 * Move a field to a new group
 * @since 3.22.1
 */
function pewc_move_field_to_new_group() {
	...
	$old_group_id = absint( $_POST['old_group_id'] );
	$new_group_id = absint( $_POST['new_group_id'] );
	$add_after = absint( $_POST['add_after'] );

	if( ! empty( $field_id ) && 'pewc_field' === get_post_type( $field_id ) && 'pewc_group' === get_post_type( $old_group_id ) && 'pewc_group' === get_post_type( $new_group_id ) ) {
		...
		if ( $success !== $field_id ) {
			// there was an error
			wp_send_json_error( 'field_not_moved' );
		}

		// Add the field ID to the list of fields under this group
		pewc_add_field_to_group( $field_id, $new_group_id, $add_after );

		// Remove this field ID from the parent group
		$fields = get_post_meta( $old_group_id, 'field_ids', true );
		// Unset element by value
		if( ( $key = array_search( $field_id, $fields ) ) !== false ) {
		    unset( $fields[$key] );
		}
		update_post_meta( $old_group_id, 'field_ids', $fields );
...
add_action( 'wp_ajax_pewc_move_field_to_new_group', 'pewc_move_field_to_new_group' );
