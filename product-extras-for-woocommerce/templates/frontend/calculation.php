...
<<?php echo $cell_tag; ?> class="<?php echo join( ' ', $calculation_classes ); ?>" data-product-field-id="<?php echo esc_attr( $products_field_id ); ?>" data-child-product-id="<?php echo esc_attr( $child_qty_product_id ); ?>">
	...
	<?php
	$calc_value = false !== strpos( $formula, '_pdf_count' ) ? $value : ''; // 3.21.7, add back the PDF count value if we're editing a product
	printf(
		'<span class="pewc-calculation-span" id="pewc-calculation-value">%s</span><input type="hidden" class="pewc-form-field pewc-calculation-value pewc-number-field pewc-number-field-%s" id="%s" name="%s" value="%s">',
		esc_attr( $calc_value ),
