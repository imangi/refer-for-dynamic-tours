	...
	if( isset( $item['child_products'][0] ) ) {
		...
		echo '<table>';
			...
			echo '<thead><tr>';
				...
				echo '<td>&nbsp;</td>'; // 3.26.5, changed th to td because it is empty, for accessibility
				...
				foreach( $h_attributes as $h_term ) {
					...
					echo '<tr>';
						...
						foreach( $v_attributes as $v_term ) {
							...
							// 3.26.5, changed to array and added filter
							$input_attributes = array();
							if( is_object( $variation_obj ) ) {
								if ( ! $variation_obj->is_purchasable() || ! $variation_obj->is_in_stock() ) {
									$input_attributes['disabled'] = 'disabled';
								}
								// 3.25.6
								$input_attributes['data-pewc-variation-name'] = $variation_obj->get_name();
								$input_attributes['aria-label'] = strip_tags( $variation_obj->get_name() ) . ' ' . __( 'Quantity', 'pewc' );
							}
							$input_attributes = apply_filters( 'pewc_filter_date_field_attributes', $input_attributes, $variation_obj );
							$attribute_string = '';
							if( ! empty( $input_attributes ) ) {
								foreach( $input_attributes as $input_attribute=>$attr_value ) {
									$attribute_string .= $input_attribute . '="' . esc_attr( $attr_value ) . '" ';
								}
							}
							...
							if ( ! empty( $quantity_field_values[$variation_id] ) ) {
								$variation_quantity = (int) $quantity_field_values[$variation_id];
							} else if ( ! empty( $default_child_products ) && in_array( $variation_id, $default_child_products ) ) {
								// 3.26.0
								$variation_quantity = 1;
							}

							$quantity_field = sprintf(
								'<span class="pewc-quantity-wrapper"><input type="number" data-option-cost="%s" min="0" step="1" max="%s" class="pewc-grid-quantity-field" name="%s[%s]" value="%d" %s></span>',
