...
if ( pewc_get_product_tooltips() ) {
	$checkboxes_wrapper_classes[] = 'product-tooltip-enabled';
}
...
<?php if( $item['child_products'] ) {
	...
	foreach( $item['child_products'] as $child_product_id ) {
		...
		$checkbox = sprintf(
			'<div class="%s"><label for="%s"><input data-option-cost="%s" data-field-label="%s" data-stock="%s" type="checkbox" name="%s[]" id="%s" class="pewc-checkbox-form-field" value="%s" %s %s>%s<span class="pewc-theme-element"></span></label><div class="pewc-checkbox-desc-wrapper">%s<div class="pewc-radio-image-desc">%s</div></div></div>',
		...
		echo apply_filters( 'pewc_filter_checkbox', $checkbox, $child_product_id, $price, $id, $name, $item, $child_product, $checkbox_id );
