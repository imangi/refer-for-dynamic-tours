...
if( ! class_exists( 'PEWC_Product_Extra_Post_Type' ) ) {
	...
	class PEWC_Product_Extra_Post_Type {
		...
		public function enqueue_scripts( $hook ) {
			...
			if( strpos( $hook, 'page_global' ) !== false || $hook == 'pewc_product_extra_page_global' || $hook == 'toplevel_page_pewc_home' || $is_post_type || ( isset( $_GET['tab'] ) && $_GET['tab'] == 'pewc' ) || false !== strpos( $hook, 'pewc-clean-up-files' ) ) {
				...
				$deps = array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-datepicker', 'wc-enhanced-select' );

				// 3.26.18
				if ( version_compare( WC_VERSION, '10.3', '>=' ) ) {
					$deps[] = 'wc-select2';
					$deps[] = 'wc-jquery-tiptip';
				} else {
					$deps[] = 'select2';
					$deps[] = 'jquery-tiptip';
				}
		...
		*/
		...
		public function metabox_checkbox_output( $post, $field ) {
			$metabox_attributes_array = apply_filters( 'pewc_metabox_checkbox_output_attributes', array(), $post, $field );
			$metabox_attributes = '';
			if ( ! empty( $metabox_attributes_array ) ) {
				foreach ( $metabox_attributes_array as $attr_name => $attr_value ) {
					if ( ! empty( $metabox_attributes ) ) {
						$metabox_attributes .= ' ';
					}
					$metabox_attributes .= esc_attr( $attr_name ) . '="' . esc_attr( $attr_value ) . '"';
				}
			}
			$value = get_post_meta( $post->ID, $field['ID'], true );
			$class = ! empty( $field['class'] ) ? $field['class'] : '';
		...
		public function metabox_product_extra_fields_output( $post, $field ) {
			...
			$groups = apply_filters( 'pewc_filter_metabox_aou_groups', $groups, $post->ID, $field ); // 3.24.7
			if( ! empty( $groups ) ) { ?>
