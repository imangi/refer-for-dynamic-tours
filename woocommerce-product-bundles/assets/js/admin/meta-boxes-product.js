...
jQuery( function( $ ) {
	...
	var $shipping_data_container     = $bundled_products_panel.parent().find( '#shipping_product_data' ),
		...
		$bundle_type_options         = $bundle_type_container.find( '.bundle_type_options button' ),
		$sold_individually_container = $( '#_sold_individually' ).closest( '.form-field' );

	$bundle_type_options.find( '.woocommerce-help-tip' ).attr( 'tabindex', -1 );
		...
		$bundled_product_search
			...
			.on( 'change', function() {
				...
				var bundled_product_ids = $bundled_product_search.val(),
				    bundled_product_id  = bundled_product_ids && bundled_product_ids.length > 0
						// Filter out empty strings to ensure only valid product IDs are considered.
						? bundled_product_ids.filter(function(id) { return id.trim() !== ""; }).shift()
						: false;
	...
	function init_bundle_shipping() {
		...
		$bundle_type_options.on( 'click', function() {
			...
			$bundle_type_options.attr( 'aria-checked', false );
		...
		$bundle_type_options.on( 'focus', function() {
			var $option = $( this );
			$bundle_type_options.attr( 'tabindex', -1 );
			$option.attr( 'tabindex', 0 );
		});
		$bundle_type_options.on( 'keydown', function( e ) {
