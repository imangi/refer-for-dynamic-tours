...
class WC_PB_Min_Max_Compatibility {
	 ...
	 *
	 * @param  array $item_data The bundled item data.
	 * @param  array $data  The posted data.
	 * @param  mixed $item_id   The bundled item ID.
	 * @param  mixed $post_id   The product ID.
	 *
	 * @return array
	 */
	public static function filter_processed_bundled_item_data( $item_data, $data, $item_id, $post_id ) {  // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	 ...
	 *
	 * @param  array  $item_data The bundled item data.
	 * @param string $context The context in which the item data is being filtered.
	...
	public static function filter_add_bundled_product_item_data( $item_data, $context, $product_id ) {
		...
		if ( 'create' === $context ) {
			...
			if ( $group_of_quantity ) {
				$item_data['quantity_min']     = $group_of_quantity;
				$item_data['quantity_max']     = $group_of_quantity;
				$item_data['quantity_default'] = $group_of_quantity;
			}
		} elseif ( 'edit' === $context ) {
			$group_of_quantity = 0;
	 ...
	 * Set global $bundled_item variable.
	 *
	 * @param  WC_Bundled_Item $bundled_item The bundled item object.
	 * @return void
	 */
	 ...
	 *
	 * @param  array  $data The quantity input arguments.
	...
	public static function restore_quantity_input_args( $data, $product ) {
		...
		if ( is_array( self::$unfiltered_args ) ) {
			...
			if ( isset( self::$unfiltered_args['min_value'] ) ) {
				if ( self::$unfiltered_args['min_value'] > 0 || 0 === self::$unfiltered_args['min_value'] ) {
					$min_qty = absint( self::$unfiltered_args['min_value'] );
				}
			} elseif ( isset( $data['min_value'] ) && ( $data['min_value'] > 0 || 0 === $data['min_value'] ) ) {
				$min_qty = absint( $data['min_value'] );
			}

			if ( isset( self::$unfiltered_args['max_value'] ) ) {
				if ( self::$unfiltered_args['max_value'] > 0 || 0 === self::$unfiltered_args['max_value'] ) {
					$max_qty = absint( self::$unfiltered_args['max_value'] );
				}
			} elseif ( isset( $data['max_value'] ) && ( $data['max_value'] > 0 || 0 === $data['max_value'] ) ) {
			...
			if ( empty( $max_qty ) || $max_qty >= $min_qty ) {
				...
				// Group of quantity may be 0, so we need to ensure it is at least 1, or the stepper block will not work.
				$data['step'] = max( $group_of_qty, 1 );
			} else {
				$data['min_value']   = $min_qty;
	 ...
	 * @param  WC_Product_Variation $bundled_variation The bundled variation.
	 * @return array
	 */
	public static function bundled_variation_data( $variation_data, $bundled_product, $bundled_variation ) {
		...
		if ( 'yes' === $bundled_variation->get_meta( 'min_max_rules', true ) ) {
	 ...
	 *
	 * @param  string $qty_meta The quantity meta.
	 * @param  string $checking_id The checking ID.
	 ...
	 * @return WC_Product
	 */
	public static function add_bundled_item_to_product( $product, $cart_item, $cart_item_key ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	 ...
	 * @return mixed
	 */
	public static function filter_bundled_item_min_quantity( $qty, $bundled_item, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	 ...
	 * @return mixed
	 */
	public static function filter_bundled_item_max_quantity( $qty, $bundled_item, $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	 ...
	 * @since 6.16.0
	 *
	 * @param  WC_Bundled_Item|int $bundled_item_or_product The bundled item or product.
	 *
	 * @return int
	 */
	public static function get_bundled_item_group_of_qty( $bundled_item_or_product ) {
		...
		if ( is_a( $bundled_item_or_product, 'WC_Bundled_Item' ) ) {
			...
			if ( isset( $item_data['allowed_variations'] ) && is_array( $item_data['allowed_variations'] ) && 1 === count( $item_data['allowed_variations'] ) ) {
				...
				$bundled_variation = $bundled_item_or_product->get_product(
					array(
						'what'   => 'min',
						'having' => 'price',
					)
