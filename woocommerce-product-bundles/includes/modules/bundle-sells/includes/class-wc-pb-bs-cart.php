...
class WC_PB_BS_Cart {
	...
	*/
	 ...
	 *
	 * @param  boolean $add     Default add-to-cart validation result.
	 ...
	 *
	 * @param  string $parent_cart_item_key Cart item key of the parent product.
	 * @param  int    $parent_id         Parent product ID.
	 * @param  int    $parent_quantity   Parent product quantity.
	 * @param  int    $variation_id      Variation ID.
	 * @param  array  $variation         Variation data.
	 * @param  array  $cart_item_data    Cart item data.
	 * @return void
	 */
	public static function bundle_sells_add_to_cart( $parent_cart_item_key, $parent_id, $parent_quantity, $variation_id, $variation, $cart_item_data ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed

		// Only proceed if the product was added to the cart via a form or query string.
		if ( empty( $_REQUEST['add-to-cart'] ) || absint( $_REQUEST['add-to-cart'] ) !== absint( $parent_id ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}
		...
		if ( ! empty( $bundle_sells_configuration ) ) {
			foreach ( $bundle_sells_configuration as $bundle_sell_id => $bundle_sell_configuration ) {

				// add some validation here:

				$bundled_item_cart_data = array( 'bundle_sell_id' => $bundle_sell_id );

				/**
				 * 'woocommerce_bundle_sell_cart_data' filter.
				 *
				 * An opportunity to copy/load child cart item data from the parent cart item data array.
				 *
				 * @param  array  $bundled_item_cart_data
				 * @param  array  $bundle_sell_configuration
				 */
				$bundled_item_cart_data = apply_filters( 'woocommerce_bundle_sell_cart_data', $bundled_item_cart_data, $bundle_sell_configuration );

				/**
				 * 'woocommerce_bundle_sell_before_add_to_cart' action.
				 *
				 ...
				 * @param  array  $bundled_item_cart_data
				 */
				do_action( 'woocommerce_bundle_sell_before_add_to_cart', $bundle_sell_configuration['product_id'], $bundle_sell_configuration['quantity'], 0, array(), $bundled_item_cart_data );

				// Add the bundle-sell to the cart.
				// No additional validation is needed, as the bundle-sell is already validated through WC_PB_Cart->validate_bundle_configuration.
				WC()->cart->add_to_cart( $bundle_sell_configuration['product_id'], $bundle_sell_configuration['quantity'], 0, array(), $bundled_item_cart_data );

				/**
				 * 'woocommerce_bundle_sell_after_add_to_cart' action.
				 *
				 * @param  int    $product_id
				 * @param  int    $quantity
				 * @param  int    $variation_id
				 ...
				 * @param  array  $bundled_item_cart_data
				 */
				do_action( 'woocommerce_bundle_sell_after_add_to_cart', $bundle_sell_configuration['product_id'], $bundle_sell_configuration['quantity'], 0, array(), $bundled_item_cart_data );

			}
		}

		self::load_bundle_sells_into_session( WC()->cart, 'add-to-cart' );
	}
	 ...
	 * @param  array  $products Products added to the cart.
	 * @return string
	 */
	public static function bundle_sells_add_to_cart_message_html( $message, $products ) {

		if ( isset( self::$bypass_filters['add_to_cart_message_html'] ) && 1 === self::$bypass_filters['add_to_cart_message_html'] ) {
			return $message;
		}
	 ...
	 * @since  6.0.0
	 *
	 * @throws Exception If invalid product data is encountered in the cart.
	 * @param WC_Cart $cart   Cart instance.
	 * @param string  $context Context.
	 *
	 * @return array|void
	 */
	public static function load_bundle_sells_into_session( $cart, $context = '' ) {
		...
		foreach ( $cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			// Expecting a product here.
			if ( ! isset( $cart_item['data'] ) || ! $cart_item['data'] instanceof WC_Product ) {
				if ( 'add-to-cart' === $context ) {
					throw new Exception( esc_html( __( 'Invalid product data encountered in cart.', 'woocommerce-product-bundles' ) ) );
				}
				continue;
			}
		 ...
		 * @since  6.6.0
		 *
		 * @param  array   $cart_item_keys  Cart item keys.
		 * @param  string  $parent_item    Parent item.
		 * @param  array   $parent_item_name Parent item name.
		 */
		...
		foreach ( $cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( ! in_array( $cart_item_key, $search_cart_item_keys, true ) ) {
				continue;
			}
		...
		foreach ( WC()->cart->cart_contents as $cart_item_key => $cart_item ) {
			...
			if ( ! in_array( $cart_item_key, $apply_to_cart_item_keys, true ) ) {
				continue;
			}
			...
			if ( isset( $bundle_sells_by_id[ $cart_item['product_id'] ] ) ) {
				...
				} elseif ( ! in_array( $cart_item_key, WC()->cart->cart_contents[ $parent_cart_item_key ]['bundle_sells'], true ) ) {
