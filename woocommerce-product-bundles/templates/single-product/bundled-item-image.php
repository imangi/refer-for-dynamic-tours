...
<div class="<?php echo esc_attr( implode( ' ', $gallery_classes ) ); ?>">
	...
	if ( has_post_thumbnail( $product_id ) ) {
		...
		$aria_label    = esc_attr( sprintf( __( 'View full screen image for %s', 'woocommerce-product-bundles' ), $bundled_item->get_title() ) );
		...
		/* translators: %1$s: Image link, %2$s: Image title, %3$s: Image rel, %4$s: Aria label, %5$s: Image */
		$html .= sprintf(
			'<a href="%1$s" class="image zoom" data-rel="%2$s" role="button" aria-haspopup="dialog" aria-label="%3$s" aria-controls="photoswipe-fullscreen-dialog">%4$s</a>',
			$image_link,
			$image_rel,
			$aria_label,
			$image
	...
	/**
	 * Filter the bundled product image HTML.
	 *
	 * @since 6.7.6
	 *
	 * @param string $html         The bundled product image HTML.
	 * @param int    $product_id   The bundled product ID.
	 * @param object $bundled_item The bundled item object.
	 */
