...
<div class="cart bundled_item_cart_content" data-title="<?php echo esc_attr( $bundled_item->get_title() ); ?>"
	...
	<table class="variations" cellspacing="0" role="presentation">
		...
		foreach ( $bundled_product_attributes as $attribute_name => $options ) {
			...
			?>
			<tr class="attribute_options <?php echo $is_attribute_value_configurable ? 'attribute_value_configurable' : 'attribute_value_static'; ?>"
				data-attribute_label="<?php echo esc_attr( wc_attribute_label( $attribute_name ) ); ?>">
				<td class="label">
					<label
						for="<?php echo esc_attr( sanitize_title( $attribute_name ) . '_' . $bundled_item->get_id() ); ?>">
						<?php
						...
						if ( $is_attribute_value_configurable ) {
							?>
							<abbr class="required" aria-hidden="true"
									title="<?php esc_attr_e( 'Required option', 'woocommerce-product-bundles' ); ?>">*</abbr>
							<?php
						}
				...
				<td class="value">
					...
					echo wc_pb_template_bundled_variation_attribute_options(
						array(
							'options'      => $options,
