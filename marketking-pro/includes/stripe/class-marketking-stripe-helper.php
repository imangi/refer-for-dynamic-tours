 ...
 * @since 4.0.0
 */
class Marketking_Stripe_Helper {
	...
	public static function get_stripe_currency( $order = null ) {
		...
		$order_id = Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? $order->id : $order->get_id();

		return Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::META_NAME_STRIPE_CURRENCY, true ) : $order->get_meta( self::META_NAME_STRIPE_CURRENCY, true );
	}
	...
	public static function update_stripe_currency( $order = null, $currency = '') {
		...
		Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, self::META_NAME_STRIPE_CURRENCY, $currency ) : $order->update_meta_data( self::META_NAME_STRIPE_CURRENCY, $currency );
	}
	...
	public static function get_stripe_fee( $order = null ) {
		...
		$amount = Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::META_NAME_FEE, true ) : $order->get_meta( self::META_NAME_FEE, true );

		// If not found let's check for legacy name.
		if ( empty( $amount ) ) {
			$amount = Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::LEGACY_META_NAME_FEE, true ) : $order->get_meta( self::LEGACY_META_NAME_FEE, true );
	...
	public static function update_stripe_fee( $order = null, $amount = 0.0 ) {
		...
		Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, self::META_NAME_FEE, $amount ) : $order->update_meta_data( self::META_NAME_FEE, $amount );
	}
	...
	public static function delete_stripe_fee( $order = null ) {
		...
		if (!Marketking_Stripe_Helper::is_wc_lt( '3.0' )){
	...
	public static function get_stripe_net( $order = null ) {
		...
		$amount = Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::META_NAME_NET, true ) : $order->get_meta( self::META_NAME_NET, true );

		// If not found let's check for legacy name.
		if ( empty( $amount ) ) {
			$amount = Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? get_post_meta( $order_id, self::LEGACY_META_NAME_NET, true ) : $order->get_meta( self::LEGACY_META_NAME_NET, true );
	...
	public static function update_stripe_net( $order = null, $amount = 0.0 ) {
		...
		Marketking_Stripe_Helper::is_wc_lt( '3.0' ) ? update_post_meta( $order_id, self::META_NAME_NET, $amount ) : $order->update_meta_data( self::META_NAME_NET, $amount );
	}
	...
	public static function get_localized_messages() {
		return apply_filters(
			'marketking_stripe_localized_messages',
			array(
				'invalid_number'           => __( 'The card number is not a valid credit card number.', 'marketking' ),
				'invalid_expiry_month'     => __( 'The card\'s expiration month is invalid.', 'marketking' ),
				'invalid_expiry_year'      => __( 'The card\'s expiration year is invalid.', 'marketking' ),
				'invalid_cvc'              => __( 'The card\'s security code is invalid.', 'marketking' ),
				'incorrect_number'         => __( 'The card number is incorrect.', 'marketking' ),
				'incomplete_number'        => __( 'The card number is incomplete.', 'marketking' ),
				'incomplete_cvc'           => __( 'The card\'s security code is incomplete.', 'marketking' ),
				'incomplete_expiry'        => __( 'The card\'s expiration date is incomplete.', 'marketking' ),
				'expired_card'             => __( 'The card has expired.', 'marketking' ),
				'incorrect_cvc'            => __( 'The card\'s security code is incorrect.', 'marketking' ),
				'incorrect_zip'            => __( 'The card\'s zip code failed validation.', 'marketking' ),
				'invalid_expiry_year_past' => __( 'The card\'s expiration year is in the past', 'marketking' ),
				'card_declined'            => __( 'The card was declined.', 'marketking' ),
				'missing'                  => __( 'There is no card on a customer that is being charged.', 'marketking' ),
				'processing_error'         => __( 'An error occurred while processing the card.', 'marketking' ),
				'invalid_request_error'    => __( 'Unable to process this payment, please try again or use alternative method.', 'marketking' ),
				'invalid_sofort_country'   => __( 'The billing country is not accepted by SOFORT. Please try another country.', 'marketking' ),
			)
		);
	}
