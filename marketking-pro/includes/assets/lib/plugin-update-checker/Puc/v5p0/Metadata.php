<?php
namespace YahnisElsts\PluginUpdateCheckerMK\v5p0;
	...
	abstract class Metadata {
		...
		protected static function createFromJson($json, $target) {
			...
			if ( empty($apiResponse) || !is_object($apiResponse) ){
				...
				//update_option('marketking_license_key_setting', '');
				...
				$failed_attempts = intval(get_option('marketking_failed_license_'.$license, 0));
				if ($failed_attempts >= 1){
					// set as inactive 
					update_option('pluginactivation_'.$email.'_'.$license.'_'.$bottom_host_name, 'inactive');
				} else {
					update_option('marketking_failed_license_'.$license, ($failed_attempts+1));
				}
				...
				update_option('marketking_failed_license_'.$license, 0);
			}
